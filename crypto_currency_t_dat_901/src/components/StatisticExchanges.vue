<template>
    <div data-aos="zoom-in" data-aos-delay="600" data-aos-once="true" class="max-w-8xl mx-auto bg-gray-700 shadow-lg relative z-20 hidden md:block">
        <div class="mb-12 xl:mb-0 mx-auto">
            <div class="relative flex flex-col min-w-0 break-words bg-gray-700 w-full mb-6 shadow-lg rounded ">
                ...
                <table class="items-center bg-transparent w-full border-collapse">
                    ...
                    <thead>
                        <tr>
                            <th class="px-6 text-gray-100 align-middle border border-solid border-sky-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">Rank</th>
                            <th class="px-6 text-gray-100 align-middle border border-solid border-sky-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">Name</th>
                            <th class="px-6 text-gray-100 align-middle border border-solid border-sky-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">Grade point</th>
                            <th class="px-6 text-gray-100 align-middle border border-solid border-sky-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">Volume (24h)</th>
                            <th class="px-6 text-gray-100 align-middle border border-solid border-sky-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">Avg. Liquidity</th>
                            <th class="px-6 text-gray-100 align-middle border border-solid border-sky-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">Weekly Visits</th>
                    ...
                    <tbody>
                        <tr v-for="(crypto, index) in cryptocurrencies.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)" class="hover:bg-gray-600">
                            <td class="border-t-0 border border-solid border-sky-100 px-6 align-center border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-gray-100">{{ crypto.rank }}</td>
                            <th class="border-t-0 px-6 align-middle border border-solid border-sky-100 text-sm border-l-0 border-r-0 whitespace-nowrap p-4 text-left flex items-center text-gray-100">
                                <img :src="crypto.image" class="mr-2 h-7"/>
                                <a :href="crypto.link" target="_blank">{{ crypto.name }}</a>
                            </th>
                            <td class="border-t-0 border border-solid border-sky-100 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4">
                                <span class="relative inline-block px-3 py-1 font-semibold text-green-500 leading-tight">
                                    <span aria-hidden class="absolute inset-0 bg-green-800 opacity-50 rounded-full"></span>
                                    <span class="relative">{{ crypto.gradepoint }}</span>
                                </span>
                            </td>
                            <td class="border-t-0 border border-solid border-sky-100 px-6 align-center border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-gray-100">{{ crypto.volume24h.BTC }}</td>
                            <td class="border-t-0 border border-solid border-sky-100 px-6 align-center border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-gray-100">{{ crypto.rating.avg }}</td>
                            <td class="border-t-0 border border-solid border-sky-100 px-6 align-center border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-gray-100">{{ crypto.rating.totalUsers }}</td>
...
<script setup>
    ...
    const loadPage = async (page) => {
        try {
            ...
            const response = await axios.get(`http://localhost:11003/api_back/cryptos/all-exchanges`);
            cryptocurrencies.value = response.data;

            // Calculer le rang en fonction de la page actuelle
            const startIndex = (page - 1) * itemsPerPage;
            cryptocurrencies.value.slice(startIndex, startIndex + itemsPerPage).forEach((crypto, index) => {
